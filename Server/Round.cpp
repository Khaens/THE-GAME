...
void Round::FirstRoundDealing(Game &game) {
  auto &m_players = game.GetPlayers();
...
bool Round::CanPlaceCard(Game &game, const Card *card, Pile *pile,
                         TurnContext &m_ctx) {
  try {
    if (!pile || !pile->GetTopCard() || !card)
      return false;
    ...
    if (m_ctx.HPplayerIndex != -1 &&
        game.GetPlayers()[m_ctx.HPplayerIndex].HPActive())
      return true;
...
Pile *Round::GetPile(int pileChoice,
                     const std::array<Pile *, PILES_AMOUNT> &piles) {
  if (pileChoice == 1)
    return piles[0];
  else if (pileChoice == 2)
    return piles[1];
  else if (pileChoice == 3)
    return piles[2];
  else if (pileChoice == 4)
    return piles[3];
  return nullptr;
}
...
int Round::GetNrOfPlayableCardsInHand(Game &game, TurnContext &m_ctx) {
  ...
  for (const auto &cardPtr : currentPlayer.GetHand()) {
    ...
    if ((CanPlaceCard(game, card, piles[0], m_ctx) ||
         CanPlaceCard(game, card, piles[1], m_ctx) ||
         CanPlaceCard(game, card, piles[2], m_ctx) ||
         CanPlaceCard(game, card, piles[3], m_ctx)) ||
        (m_ctx.HPplayerIndex != -1 &&
         m_players[m_ctx.HPplayerIndex].HPActive())) {
      count++;
    }
  }
  return count;
...
void Round::UpdateContext(Game &game, TurnContext &m_ctx,
                          Player &currentPlayer) {
  if (game.GetDeckSize() == 0)
    m_ctx.endgame = true;
  if (m_ctx.endgame || currentPlayer.GActive())
    m_ctx.baseRequired = 1;
  else
    m_ctx.baseRequired = 2;
  if (m_ctx.endgame &&
      currentPlayer.GetPlayerIndex() == m_ctx.GamblerPlayerIndex) {
    if (m_ctx.GamblerEndgamePenaltyTurns > 0 &&
        currentPlayer.GetHand().size() > 1) {
      m_ctx.currentRequired = 2;
    }
  } else if (m_ctx.TaxEvPlayerIndex != -1 &&
             m_ctx.TaxEvPlayerIndex != currentPlayer.GetPlayerIndex() &&
...
bool Round::IsGameWon(Game &game, Player &currentPlayer) {
  if (game.GetCtx().endgame && currentPlayer.GetHand().size() == 0)
